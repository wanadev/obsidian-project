...
// TODO project.addBlobFromUrl(url, callback, options)   → function(error, id)   // ? PROXY!
//
// TODO Canvas
...
var ProjectManager = SerializableClass.$extend({
    ...
    openFromLocalFile: function(callback) {
        return Q.Promise(function(resolve, reject) {
            setTimeout(function() {
                reject(new Error("NotImplementedError"));    // FIXME
            }, 0);
        }).nodeify(callback);
    },
    ...
    openFromUrl: function(url, callback) {
        var this_ = this;
        return helpers.httpGet(url)
            .then(function(response) {
                this_.openFromBuffer(response.buffer);
            }).nodeify(callback);
    },
    ...
    addBlob: function(blob, options, callback) {
        ...
        return Q.Promise(function(resolve, reject) {
            ...
            reader.onload = function(event) {
                try {
                    var buffer = new Buffer(event.target.result);
                    var id = this_.addBlobFromBuffer(buffer);
    ...
    addBlobFromUrl: function(buffer, options, callback) {
        return Q.Promise(function(resolve, reject) {
            setTimeout(function() {
                reject(new Error("NotImplementedError"));    // FIXME
            }, 0);
    ...
    getBlob: function(id) {
        ...
        var blob = helpers.createBlob([buffer.toArrayBuffer()], {type: mime});
