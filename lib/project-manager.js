...
var ProjectManager = SerializableClass.$extend({
    ...
    openFromUrl: function(url, callback) {
        var this_ = this;
        var p = httpRequest.getRawProxy(url)
     ...
     * @return {Promise.<String>} The blob id.
     */
    ...
    addBlobFromBuffer: function(buffer, options) {
        options = options || {};
        var id = options.id || helpers.uuid4();
    ...
    addBlobFromUrl: function(url, options, callback) {
        options = options || {};
        var this_ = this;
        var p = httpRequest._operations._requestProxy(url)
